...
architecture RTL of FSM_read_config is
    ...
    constant Config_End_Padding_Word_Count : natural := 2;
    constant Standard_Energy_Threshold_Count : natural := 6;
    constant Config_Register_Count         : natural := 8;
    constant Config_Read_Word_Count        : natural := (32 * Filter_Number * Detector_Number) + Config_Register_Count + (Filter_Number * Detector_Number) + (Standard_Energy_Threshold_Count * Filter_Number * Detector_Number) + Config_End_Padding_Word_Count;
    signal add_coef : integer;

    type state_type_config is (IDLE, read_low_frequency, valid_low_frequency, read_high_frequency, valid_high_frequency, read_control_config, valid_read_control_config, read_trigger, valid_read_trigger,
            ...
            case state_config is
                ...
                when read_high_frequency =>
                    ...
                    if add_coef < 32 then
                        ...
                        if cnt_number_detector = Detector_Number - 1 then
                            state_config           <= read_control_config;
                            o_pipe_in_config_rd_en <= '0';
                            o_coef_fir_ready       <= '1';
                            add_coef               <= 0;
                            cnt_number_detector    <= 0;
                ...
                when read_control_config =>

                    o_pipe_in_config_rd_en <= '1';
                    state_config           <= valid_read_control_config;

                when valid_read_control_config =>

                    o_pipe_in_config_rd_en <= '0';
                    if i_pipe_in_config_valid = '1' then
                        o_reg_config(0) <= std_logic_vector(i_pipe_in_config_dout(15 downto 0));
